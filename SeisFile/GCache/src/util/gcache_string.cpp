...
bool WildcardMatch(const char * pat, const char* str, bool case_sensitive) {
    ...
    loopStart: for (s = str, p = pat; *s; ++s, ++p) {
        switch (*p) {
            case '?':
                if (*s == '.') goto starCheck;
                break;
            case '*':
                ...
                goto loopStart;
            default:
                if (table[(int) *s] != table[(int) *p]) goto starCheck;
                break;
        }/*switch*/
    }/*for*/
    ...
    starCheck: if (!star) return false;
    str++;
    goto loopStart;
}
...
Error::Error(const std::string& msg, const std::string& file, int line,
        const std::string& function) {
    char lineStr[64];
    sprintf(lineStr, "%d", line);

    errstr = msg + " at " + file + ":" + lineStr + " in " + function;
}
...
short ToShort(const std::string& val) {
    ...
    ASSERT(num == 1, "Problem converting " + val + " to short integer.");
    return result;
}
...
int ToInt(const string& val) {
    ...
    ASSERT(num == 1, "Problem converting " + val + " to integer.");
    return result;
}
...
int64_t ToInt64(const string& val) {
    int64_t result = 0;
    char trash;
    sscanf(val.c_str(), "%ld%c", &result, &trash);
...
float ToFloat(const string& val) {
    ...
    ASSERT(num == 1, "Problem converting " + val + " to float.");
    return result;
}
...
double ToDouble(const string& val) {
    ...
    ASSERT(num == 1, "Problem converting " + val + " to double.");
    return result;
}
...
bool ToBool(const string& val) {
    ...
    if (newVal == "true") {
        ...
        ASSERT(false, "Problem converting " + val + " to boolean.");
        return false;
    }
}
...
string QuoteString(const string& str, const char* deliminators) {

    string result(str);
    for (int i = result.length() - 1; i >= 0; --i) {
        char ch = result[i];
        if (!isprint(ch) || ch == '\\' || strchr(deliminators, ch)) {
            switch (ch) {
                case '\\':
                    result.replace(i, 1, "\\\\");
                    break;
        ...
        if (current + 1 < result.size()) {
            ...
            if (isxdigit(result[current + 1])) {
                ...
                ASSERT(current + num_chars < result.size(),
                        "escape pattern \\<hex><hex> is missing second digit in '" + str + "'");
                char sub_str[3];
...
bool EndsWith(const std::string& str, const std::string endStr) {
    ...
    if (strLen < endStrLen) return false;
...
bool StartsWith(const std::string& str, const std::string startStr) {
    if (str.size() < startStr.size()) return false;
