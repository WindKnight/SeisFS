 ...
 * @return              no return
 */
GarbageCollectorNas::GarbageCollectorNas() {
}
 ...
 * @return              no return.
 */
void GarbageCollectorNas::ListDirsFiles(string dir_path, vector<string>& fileList,
 ...
 * @return              no return.
 */
void GarbageCollectorNas::GetNodeInformation(TreeNode* &node, string path, TreeNode* parent) {
    if (-1 == access(path.c_str(), 0))   //路径不存在
            {
        return;
    }
 ...
 */
TreeNode* GarbageCollectorNas::GetChildByOrder(TreeNode* parent, int childOrder) {
 ...
 */
bool GarbageCollectorNas::LifetimeExpired(string fileName, int lifetime) {
    struct timeval now;
 ...
 */
bool GarbageCollectorNas::ParseParam() {
    char *confile = getenv("GCACHE_TMP_NAS_CONF");
    if (confile == NULL) {
        cout << "ERROR:environment path GCACHE_TMP_NAS_CONF doesn't exist" << endl;
        return false;
    }
    string para_filename = confile;
    cout << "nas para_filename = " << para_filename << endl;
 ...
 */
bool GarbageCollectorNas::PostOrderTraversal() {
    cout << "nas garbage collection start" << endl;
        ...
        while (nodeStack.size() > 0 && flag) {
            ...
            if (rootNode->DirectoryList.size() == (unsigned int) rootNode->CurrentChildOrder) //all the directory in current node have been traversed
                ...
                for (unsigned int i = 0; i < rootNode->FileList.size(); i++) {
                    ...
                    int read_len = sizeof(MetaTMPNAS);
                    MetaTMPNAS* metaData = new MetaTMPNAS();
