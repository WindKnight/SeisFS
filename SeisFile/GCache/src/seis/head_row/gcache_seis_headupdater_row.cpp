/*
 * gcache_seis_headupdater_row.cpp
 *
 ...
 * @return              no return.
 */
SeisHeadUpdaterRow::SeisHeadUpdaterRow(hdfsFS fs, const std::string& hdfs_data_name,
 ...
 * @return              no return.
 */
SeisHeadUpdaterRow::~SeisHeadUpdaterRow() {
    delete[] _head_offset;
    Close();
}
 ...
 */
bool SeisHeadUpdaterRow::SetHeadFilter(const HeadFilter& filter) {
    if (_is_close_flag) {
    ...
    if (_key_list.size() > 0 && _key_list.size() < _meta->head_length && !_is_update_part_flag) {
        ...
        for (int64_t i = 0; i < _meta->head_dir_size; ++i) {
            SeisSubHeadReaderRow* reader = new SeisSubHeadReaderRow(_fs, _hdfs_data_name, _meta, i);
            if (!reader->Init()) {
                delete reader;
                return false;
            }
...
bool SeisHeadUpdaterRow::SetRowFilter(const RowFilter& filter) {
    if (_is_close_flag) {
        Err("object is closed, not to be used.\n");
        return false;
    }
 ...
 */
//int SeisHeadUpdaterRow::GetKeys() {
//    if (_is_close_flag) {
//        Err("object is closed, not to be used.\n");
//        return -1;
//    }
 ...
 */
bool SeisHeadUpdaterRow::Seek(int64_t head_idx) {
    if (_is_close_flag) {
        Err("object is closed, not to be used.\n");
        return false;
    }
 ...
 */
bool SeisHeadUpdaterRow::Put(const void* head) {
    if (_is_close_flag) {
        Err("object is closed, not to be used.\n");
        return false;
    }
 ...
 * @return              return true if put successfully, else return false.
 */
bool SeisHeadUpdaterRow::Put(int64_t head_idx, const void * head) {
    if (_is_close_flag) {
        Err("object is closed, not to be used.\n");
        return false;
    }
 ...
 */
bool SeisHeadUpdaterRow::AutoCompact() {
    ...
    SeisHeadCompactorRow* headCompactor = new SeisHeadCompactorRow(_fs, _hdfs_data_name, _meta);
    ...
    for (int64_t i = 0; i < _meta->head_dir_size; ++i) {
        SeisSubHeadUpdaterRow* updater = new SeisSubHeadUpdaterRow(_fs, _hdfs_data_name, _meta, i);
        if (!updater->Init()) {
            delete updater;
            return false;
        }
