...
#define COMPACT_DATA_SIZE 256*1024*1024
 ...
 * @return              no return.
 */
SeisSubTraceCompactor::SeisSubTraceCompactor(hdfsFS fs, const std::string& hdfs_data_name,
 ...
 * @return              no return.
 */
SeisSubTraceCompactor::~SeisSubTraceCompactor() {
...
bool SeisSubTraceCompactor::Compact() {
    ...
    for (int64_t i = 0; i < (int64_t) _index_vector_array.size(); i++) {
        ...
        if (_index_vector_array[i]) {
            ...
            int64_t left_rows = listFileInfo[i].mSize / _meta->trace_row_bytes;
            int64_t roll = 0;
            while (left_rows > 0) {
                int64_t read_rows = left_rows > max_read_rows ? max_read_rows : left_rows;
                int64_t read_size = read_rows * _meta->trace_row_bytes;
                ...
                if (row_part_vector[roll]) {
                    for (uint64_t j = 0; j < row_part_vector[roll]->size(); j++) {
                        ...
                        int64_t upd_dat_offset =
                                _meta->trace_row_bytes
                                        * ((*row_part_vector[roll])[j]->second.second
                                                % _meta->file_max_rows); //indicate the offset of the trace that user need
                        ...
                        char* traceRow = new char[_meta->trace_row_bytes];
                        int64_t ret_read = SafeRead(_fs, upd_dat_fd, traceRow,
                                _meta->trace_row_bytes);
                        ...
                        memcpy(newFileBuff + write_row_index * _meta->trace_row_bytes, traceRow,
                                _meta->trace_row_bytes);
                        delete[] traceRow;
                    }
                }
    ...
    std::string trace_dir = _hdfs_data_name + "/" + _trace_dir_uuid;
    std::string hdfs_idx_dir = trace_dir + "/" + "Update_Index";
    std::string hdfs_dat_dir = trace_dir + "/" + "Update_Data";
    std::string hdfs_trun_dir = trace_dir + "/" + "Truncate_Data";
    ...
    if (!CreateAllDir(_fs, _hdfs_data_name, _trace_dir_uuid.c_str())) {
...
bool SeisSubTraceCompactor::LoadIndex() {
    std::vector<std::pair<int64_t, std::pair<int, int64_t>>> index_vector;
    std::string trace_uuid_dir = _hdfs_data_name + "/" + _trace_dir_uuid;
    std::vector<std::pair<int64_t, int64_t>> trun_vector;
