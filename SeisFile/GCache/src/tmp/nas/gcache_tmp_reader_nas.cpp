 ...
 * @return                no return
 */
ReaderNAS::ReaderNAS(int fd, bool direct_read, char *map_buf) :
...
bool ReaderNAS::Init() {
    ...
    if (_direct_read) {
        ...
        if (0 == posix_memalign((void **) &_direct_read_buf, _block_size, _direct_buf_len)) {
            return true;
        } else {
            return false;
        }
 ...
 */
int64_t ReaderNAS::Read(char *data, int64_t max_len) {
    int64_t ret_len;
    if (_map_buf == NULL) {
        if (!_direct_read) {
 ...
 * @return                return the number of bytes read, if an error occurs, return -1.
 */
int64_t ReaderNAS::ReadLine(char* s, int size) {
    char c = 0;
 ...
 * @return                return the number of bytes read, if an error occurs, return -1.
 */
int64_t ReaderNAS::ReadLine(std::string& s) {
    s.clear();
    char c = 0;
    int iRet = 0;
    while (1) {
 ...
 */
bool ReaderNAS::Seek(int64_t offset, int whence) {
    if (_map_buf == NULL) {
        if (!_direct_read) {
