...
HeadReaderNAS::~HeadReaderNAS() {
	delete _gf_head;
	delete _gf_head_next;
}

HeadReaderNAS::HeadReaderNAS(const std::string& filename, MetaNAS *meta){
	_meta = meta;
...
bool HeadReaderNAS::Init(){
	...
	HeadReaderNAS::SetHeadFilter(init_head_filter);
	HeadReaderNAS::SetRowFilter(init_row_filter);
 ...
 */
bool HeadReaderNAS::SetHeadFilter(const HeadFilter& head_filter) {
 ...
 * caller should not delete the pointer of filter after calling this function
 */
bool HeadReaderNAS::SetRowFilter(const RowFilter& row_filter) {
	_row_filter = row_filter;
	return true;
}

int HeadReaderNAS::GetHeadSize() {
	_head_type = _meta->MetaGetheadtype();
...
int64_t HeadReaderNAS::GetTraceNum() {
	int64_t tracenum = 0;
	int64_t temp = 0;
 ...
 */
bool HeadReaderNAS::Seek(int64_t trace_num) {
	_cur_logic_num = trace_num;
	int64_t head_pos = 0;
	int64_t realnum = HeadReaderNAS::GetRealNum(trace_num);
...
int64_t HeadReaderNAS::Tell() {
	return _cur_logic_num;
}

bool HeadReaderNAS::Get(int64_t trace_num, void* head) {
 ...
 */
bool HeadReaderNAS::HasNext() {
	if(HeadReaderNAS::Tell() < HeadReaderNAS::GetTraceNum()){ //modify equal
		return true;
	}
	return false;
}
...
bool HeadReaderNAS::Next(void* head) {
	if(HeadReaderNAS::HasNext()){
		HeadReaderNAS::Seek(_cur_logic_num);
		char *p2 = (char *)head;
		int64_t offset_in_head = 0;
		int64_t one_key_size = 0;
		char *total_head = new char[_meta->MetaGetheadlength()];
...
int64_t HeadReaderNAS::GetRealNum(int64_t trace_num){
	int64_t realnum;
	if(trace_num >= _meta->MetaGettracenum()){
		std::cout<<"trace index out of range"<<endl;
		return -1;
