...
TraceUpdaterNAS::TraceUpdaterNAS(const std::string& filename){
	...
	_filename = filename;
	TraceUpdaterNAS::Init();
}

bool TraceUpdaterNAS::Init(){
...
bool TraceUpdaterNAS::SetRowFilter(const RowFilter& filter){
	_row_filter = filter;
...
bool TraceUpdaterNAS::Seek(int64_t trace_num){
...
bool TraceUpdaterNAS::Put(const void* trace){
...
bool TraceUpdaterNAS::Put(int64_t trace_num, const void* trace){
	if(TraceUpdaterNAS::Seek(trace_num)){
		TraceUpdaterNAS::Put(trace);
	}
	else{
		return false;
	}
...
bool TraceUpdaterNAS::Sync(){
	_gf_trace->Flush();
	return true;
}

bool TraceUpdaterNAS::Close(){
	_gf_trace->Close();
	delete _gf_trace;
	delete _meta;
	return true;
