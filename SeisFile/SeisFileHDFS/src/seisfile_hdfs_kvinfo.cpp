...
namespace file {
	KVInfoHDFS::~KVInfoHDFS() {

	}

	KVInfoHDFS::KVInfoHDFS(const std::string& filename) {
		_filename = filename;
	...
	bool KVInfoHDFS::Put(const std::string &key, const char* data, uint32_t size){
#if 0
		...
		KVInfoHDFS::KV_MetaWrite(key.length(), size);
		return true;
#endif
		return true;
	}

	bool KVInfoHDFS::Delete(const std::string &key){
#if 0
		...
		KVInfoHDFS::KV_GetCurrentMetaInfo();
		_valid_kvinfo_num--;
		KVInfoHDFS::KV_UpdateCurrentMetaInfo();

		float judge = _valid_kvinfo_num / _kvinfo_num;
		if(judge < 0.8){
			KVInfoHDFS::KV_Update();
		}
	...
	bool KVInfoHDFS::Get(const std::string &key, char*& data, uint32_t& size){
#if 0
		map<string, string>::iterator it;
		it = _kv_map.find(key);
		if(it == _kv_map.end()){
	...
	uint32_t KVInfoHDFS::SizeOf(const std::string &key){
#if 0
		map<string, string>::iterator it;
		it = _kv_map.find(key);
		if(it == _kv_map.end()){
	...
	bool KVInfoHDFS::Exists(const std::string &key){
#if 0
		map<string, string>::iterator it;
		it = _kv_map.find(key);
		if(it == _kv_map.end()){
	...
	bool KVInfoHDFS::GetKeys(std::vector<std::string> &Keys){
#if 0
		map<string, string>::iterator it;
	...
	void KVInfoHDFS::KV_MetaWrite(uint32_t key_length, uint32_t value_length){
#if 0
		KVInfoHDFS::KV_GetCurrentMetaInfo();
		_kvinfo_num++;
		_valid_kvinfo_num++;
	...
	void KVInfoHDFS::KV_Update(){
		...
		/*std::string temp_key_filename = SEISHDFS_HOME_PATH + _filename + "_keytemp";
		std::string temp_value_filename = SEISHDFS_HOME_PATH + _filename + "_valuetemp";
		std::string temp_meta_filename = SEISHDFS_HOME_PATH + _filename + "_metatemp";
		GEFile *key_temp = new GEFile(temp_key_filename);
		GEFile *value_temp = new GEFile(temp_value_filename);
		GEFile *meta_temp = new GEFile(temp_meta_filename);*/
	...
	void KVInfoHDFS::KV_GetCurrentMetaInfo(){
#if 0
		char p[sizeof(uint32_t)];
		_gf_kvmeta->SeekToBegin();
	...
	void KVInfoHDFS::KV_UpdateCurrentMetaInfo(){
#if 0
		char p[sizeof(uint32_t)];
		_gf_kvmeta->SeekToBegin();
