...
/*ReaderHDFS::ReaderHDFS(const std::string& filename) {
...
ReaderHDFS::ReaderHDFS(const std::string& filename, MetaHDFS *meta) {
	_meta = meta;
...
//	_gf_trace_next->SetProjectName(_trace_filename);
	...
	ReaderHDFS::SetHeadFilter(init_head_filter);
	ReaderHDFS::SetRowFilter(init_row_filter);
 ...
 */
bool ReaderHDFS::SetHeadFilter(const HeadFilter& head_filter) {
 ...
 * caller should not delete the pointer of filter after calling this function
 */
bool ReaderHDFS::SetRowFilter(const RowFilter& row_filter) {
	_row_filter = row_filter;
	return true;
}


int ReaderHDFS::GetTraceSize() {
	_trace_type = _meta->MetaGettracetype();
	return _trace_type.trace_size;
}

int ReaderHDFS::GetHeadSize() {
	_head_type = _meta->MetaGetheadtype();
...
uint64_t ReaderHDFS::GetTraceNum() {
	int64_t tracenum = 0;
	int64_t temp = 0;
 ...
 */
bool ReaderHDFS::Seek(int64_t trace_num) {
	/*if(trace_num > ReaderHDFS::GetTraceNum() - 1){
	...
	_cur_logic_num = trace_num;
	int64_t head_pos = 0, trace_pos = 0;
	int64_t realnum = ReaderHDFS::GetRealNum(trace_num);
...
//	_gf_trace_next->Seek(trace_pos, SEEK_SET);
	lseek(fd_head_next_, head_pos, SEEK_SET);
...
bool ReaderHDFS::Get(int64_t trace_num, void *head, void* trace){
 ...
 */
bool ReaderHDFS::HasNext() {
	//printf("current trace num is:%d\n",ReaderHDFS::Tell());
	if(ReaderHDFS::Tell() < ReaderHDFS::GetTraceNum()){ //modify equal
		return true;
	}
	return false;
}

bool ReaderHDFS::Next(void* head, void* trace){ //Next(void* head) {
	if(ReaderHDFS::HasNext()){
		//return ReaderHDFS::Get(ReaderHDFS::Tell()/* + 1*/, head, trace);  //plus 1
		ReaderHDFS::Seek(_cur_logic_num);
		char *p2 = (char *)head;
		int64_t offset_in_head = 0;
		int64_t one_key_size = 0;
		char *total_head = new char[_meta->MetaGetheadlength()];
...
int64_t ReaderHDFS::GetRealNum(int64_t trace_num){
	int64_t realnum;
	if(trace_num >= _meta->MetaGettracenum()){
		//error:trace index out of range
		//printf("tracenum is:%u and total is:%u\n", trace_num, _meta->MetaGettracenum());
